...
export const navigateToScreenFromNotification = ({ data, isDarkMode }: { data: any, isDarkMode: boolean }, doNavigate: (map: any) => void, spinner: () => void) => {
    ...
    if (typeNumber === CONST.FCM_NEW_DOCTOR_CONFIRMATION) {  // FromDoctorToAdmin
        fetchDoctorDocument(id, (doctor) => {
            doNavigate({ navigatorTag: CONST.DOCTOR_DETAIL_ADMIN, data: { data: doctor.asJsonAll(), isDark: isDarkMode } })
        }, spinner)
        ...
        fetchExaminationDocument(id, (examination) => {
            doNavigate({ navigatorTag: CONST.APPOINTMENT_SCREEN, data: examination.asJsonAll() })
        }, spinner)
    } else if (typeNumber === CONST.FCM_APPOINTMENT_REMINDER_FOR_DOCTOR) { // ToDoctor
        fetchExaminationDocument(id, (examination) => {
            doNavigate({ navigatorTag: CONST.APPOINTMENT_SCREEN_DOCTOR, data: examination.asJsonAll() })
        }, spinner)
        ...
        findPref(CONST.USER_ID_AUTH, (userAuthID) => {
            doNavigate({ navigatorTag: CONST.VIDEO_CALL_SCREEN, data: { token: data.id, room: data.navigator, userID: userAuthID, userName: data.time } })
        }, spinner)
    }
}
